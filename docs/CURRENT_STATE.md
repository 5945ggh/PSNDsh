...
- 多个相互隔离账号的注册、登录、退出、会话恢复和资料编辑；业务数据按用户隔离。
- `REGISTRATION_MODE=first-user|open|closed` 注册策略。
- 嵌套条目、完成模式、状态、截止时间、归档和树形管理。
- 周计划的结构化条目纳入、移除和幂等结转；周计划项支持 `focus` / `commitment` 角色，`focus` 可记录预计投入时间。
- 本周清单使用自由 Markdown 文本，与条目树不自动转换或双向同步；新周创建时清单原样复制。
- 实时专注、手动补录、重叠校验、片段拆分和未关联统计。
- 日历范围查询、手工日程、日程模板、ICS 预览确认导入、来源重复更新差异预览和导入批次管理。
- 计算型周度回顾：默认并列上周复盘与本周进展，支持只读查看历史周的计划与统计；复用周计划、条目状态、周投入统计、未关联时长和周备注，不新增复盘历史实体。上周的“已带入下一周”依据下一周计划中实际记录的 `rollover` 条目，不从当前状态重新推断。
- 日、周、月统计，直接投入、聚合投入和未关联投入区分。
- 首页聚合、季节名句本地内容包、天气不可用降级状态。
- 当前账号 JSON 导出。
- 账目快捷捕获 API、Inbox 待整理队列和全部记录历史流。
- 账目支持发生时间/发生日期、记录时间、备注、分类、支付方式、标签和 `pending` / `reviewed` 整理状态。
- Inbox 支持“保存修改”（保留待整理）与“保存并标记已整理”（移出 Inbox），保存失败保留草稿。
- 全部记录使用按 `effectiveTimezone` 排序的服务端 cursor 分页；历史游标绑定 revision，数据变化后要求重新加载。
- 分类、支付方式和标签支持创建、改名、归档、恢复与合并。
- 展开账目详情支持左右方向键切换记录；上下方向键保留页面滚动行为。
...
- 认证支持多个相互隔离账号，但不包含协作、共享、角色权限或管理员能力。
...
- SQLite 与进程内认证限流按单实例部署设计；上云前应配置稳定的 `AUTH_SECRET`、最终 HTTPS 的 `PUBLIC_ORIGIN`，并安排定期备份和恢复演练。
