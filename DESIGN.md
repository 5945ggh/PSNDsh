...
- 账目按任务分模式：Inbox 是逐条整理队列；全部记录是可扫描、可分页的完整宽度列表，详情只有主动选中后在当前行下方原地展开，桌面与窄屏保持同一种交互
...
- Expense list performance: Inbox 与全部记录均使用服务端分页；列表只渲染当前页，切换详情不重置分页位置；表格行的标签仅展示有限摘要，详情中展示完整标签
...
- 账目表格的默认每页条数与筛选条件仍可根据真实记录规模调整；首版固定提供明确的页码和上一页/下一页操作。
